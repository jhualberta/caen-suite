...
#else
    // NOTE: if you aren't on WINXP, you can use GetTickCount64()
    // to reach a greater value for the elapsed time. Right now
    // it is around 49 days.
    res = (uint64_t)GetTickCount();
#endif
    return res;
}
...
int32_t InitDgtzParams(CAENDPP_DgtzParams_t *Params) {
    ...
    for (ch = 0; ch < MAX_BOARD_CHNUM; ch++) {
        ...
        Params->DPPParams.X770_extraparameters[ch].SaturationHoldoff = 300;
...
int32_t GetModifiedChannelParameters(int32_t ch, CAENDPP_DgtzParams_t *Params) {
    ...
    while(quit == 0) {
        ...
        printf("-------------- X770 PARAMS --------------\n");
        printf("1 : Transistor reset enabled\t\t\t= %u\n", Params->ResetDetector[ch].Enabled);
        printf("2 : Input impedance\t\t\t\t= %d\n", Params->DPPParams.X770_extraparameters[ch].InputImpedance);
        printf("3 : Change input dynamic\t\t\t\t\t= %d\n", Params->DPPParams.X770_extraparameters[ch].CRgain); // TODO display right value in Vpp
        printf("4 : Trans. Reset gain\t\t\t\t\t= %d\n", Params->DPPParams.X770_extraparameters[ch].TRgain);
        printf("5 : Saturation holdoff\t\t\t\t= %d\n", Params->DPPParams.X770_extraparameters[ch].SaturationHoldoff);
		printf("---- RESET ----\n");
        printf("6 : Internal Reset Detection Enable\t\t\t\t= %d\n", Params->ResetDetector[ch].ResetDetectionMode);
        printf("7 : Reset Threhold\t\t\t\t= %d\n", Params->ResetDetector[ch].thrhold);
        printf("8 : Reset minimum length\t\t\t= %d\n", Params->ResetDetector[ch].reslenmin);
        printf("9 : Reset hold off\t\t\t\t= %d\n", Params->ResetDetector[ch].reslength);
		printf("---- TRIGGER ----\n");
        ...
        case 'o':
            ...
            break;
        case '1':
			printf("Transistor Reset detector:\n\n 1) Disabled 2) Enabled\n\n ");
			c = _getch();
            if (c == '1')
                Params->ResetDetector[ch].Enabled = 0;
            else if (c == '2')
                Params->ResetDetector[ch].Enabled = 1;
            break;
		case '2':
            ...
            break;
        case '3':
            printf("Enter Input dynamic (Vpp):\n\n 1) 10.0   2)5.0   3) 2.5   4) 1.25\n\n");
			c = _getch();
            if (c == '1')
                Params->DPPParams.X770_extraparameters[ch].CRgain = 0;
            else if (c == '2')
                Params->DPPParams.X770_extraparameters[ch].CRgain = 1;
			else if (c == '3')
                Params->DPPParams.X770_extraparameters[ch].CRgain = 2;
            else if (c == '4')
                Params->DPPParams.X770_extraparameters[ch].CRgain = 3;
            break;
        case '4':
            printf("Enter Transient Reset gain: ");
            if (scanf("%d", &d32) == 1)
                Params->DPPParams.X770_extraparameters[ch].TRgain = d32;
            break;
		case '5':
            printf("Enter Saturation Hold off: (us)");
            if (scanf("%f", &f32) == 1)
                Params->DPPParams.X770_extraparameters[ch].SaturationHoldoff = (int32_t)(f32 * 1000.0);
            break;
		case '6':
			printf("Enter Reset Detection mode:\n\n 1) Internal 2) GPIO\n\n ");
			c = _getch();
            if (c == '1')
                Params->ResetDetector[ch].ResetDetectionMode = CAENDPP_ResetDetectionMode_Internal;
            else if (c == '2')
                Params->ResetDetector[ch].ResetDetectionMode = CAENDPP_ResetDetectionMode_GPIO;
            break;
		case '7':
            printf("Enter Reset minimum len: (us)");
            if (scanf("%d", &f32) == 1)
                Params->ResetDetector[ch].thrhold = (int32_t)(f32 * 1000.0);
            break;
		case '8':
            printf("Enter Reset threshold: (us)");
            if (scanf("%d", &f32) == 1)
                Params->ResetDetector[ch].reslenmin = (int32_t)(f32 * 1000.0);
            break;
		case '9':
            printf("Enter Reset holdoff: (us)");
            if (scanf("%d", &f32) == 1)
               Params->ResetDetector[ch].reslength = (int32_t)(f32 * 1000.0);
            break;
		case 'U':
			printf("Trigger Mode:\n\n 1) On fast trapezoid 2) Unsupported\n\n ");
			c = _getch();
            if (c == '1')
                Params->DPPParams.X770_extraparameters[ch].trigMODE = 0;
            else if (c == '2')
...
void printAcquisitionInterface() {
    ...
    printf("G : Get Board Configuration\n");
...
DPPDemo_Cmd_t getACQCommad(int32_t c) {
    switch (c) {
        ...
        case 'C': return DPPDemo_Cmd_Configuration_Set;
        case 'G': return DPPDemo_Cmd_Configuration_Get;
...
/// 
/// \param   [IN] c   : The character to convert
DPPDemo_Cmd_t getHVCommad(int32_t c) {
    switch (c) {
...
/// 
/// \param   [IN] c   : The character to convert
DPPDemo_Cmd_t getCommad(int32_t c) {
    DPPDemo_Cmd_t res;
    ...
    case 'I': return DPPDemo_Cmd_AttachBoards;
...
/// \fn      char *getInputRangeString(CAENDPP_InputRange_t inRange, char dest[]);
/// \brief   Converts to string the given Input Range
/// 
/// \param   [IN] inRange   : The Input Range value to convert
/// \param   [OUT] dest     : The string to fill.
/// 
/// \return  a pointer to the filled string.
char *getInputRangeString(CAENDPP_InputRange_t inRange, char dest[]) {
    switch (inRange) {
    case CAENDPP_InputRange_9_5Vpp: strcpy(dest, "9.5 Vpp"); break;
    case CAENDPP_InputRange_3_7Vpp: strcpy(dest, "3.7 Vpp"); break;
    case CAENDPP_InputRange_1_4Vpp: strcpy(dest, "1.4 Vpp"); break;
    case CAENDPP_InputRange_0_6Vpp: strcpy(dest, "0.6 Vpp"); break;
    case CAENDPP_InputRange_3_0Vpp: strcpy(dest, "3.0 Vpp"); break;
    case CAENDPP_InputRange_1_0Vpp: strcpy(dest, "1.0 Vpp"); break;
    case CAENDPP_InputRange_0_3Vpp: strcpy(dest, "0.3 Vpp"); break;
    case CAENDPP_InputRange_10_0Vpp: strcpy(dest, "10.0 Vpp"); break;
    case CAENDPP_InputRange_5_0Vpp: strcpy(dest, "5.0 Vpp"); break;
    case CAENDPP_InputRange_2_0Vpp: strcpy(dest, "2.0 Vpp"); break;
    case CAENDPP_InputRange_0_5Vpp: strcpy(dest, "0.5 Vpp"); break;
    case CAENDPP_InputRange_2_5Vpp: strcpy(dest, "2.5 Vpp"); break;
    case CAENDPP_InputRange_1_25Vpp: strcpy(dest, "1.25 Vpp"); break;
    case CAENDPP_InputRange_0_1Vpp: strcpy(dest, "0.1 Vpp"); break;
    case CAENDPP_InputRange_0_21Vpp: strcpy(dest, "0.21 Vpp"); break;
    case CAENDPP_InputRange_0_45Vpp: strcpy(dest, "0.45 Vpp"); break;
    case CAENDPP_InputRange_0_83Vpp: strcpy(dest, "0.83 Vpp"); break;
    case CAENDPP_InputRange_1_6Vpp: strcpy(dest, "1.6 Vpp"); break;
    case CAENDPP_InputRange_3_3Vpp: strcpy(dest, "3.3 Vpp"); break;
    case CAENDPP_InputRange_6_6Vpp: strcpy(dest, "6.6 Vpp"); break;
    case CAENDPP_InputRange_13_3Vpp: strcpy(dest, "13.3 Vpp"); break;
    case CAENDPP_InputRange_X0_25: strcpy(dest, "X0.25"); break;
    case CAENDPP_InputRange_X0_5: strcpy(dest, "X0.5"); break;
    case CAENDPP_InputRange_X1: strcpy(dest, "X1"); break;
    case CAENDPP_InputRange_X2: strcpy(dest, "X2"); break;
    case CAENDPP_InputRange_X4: strcpy(dest, "X4"); break;
    case CAENDPP_InputRange_X8: strcpy(dest, "X8"); break;
    case CAENDPP_InputRange_X16: strcpy(dest, "X16"); break;
    case CAENDPP_InputRange_X32: strcpy(dest, "X32"); break;
    case CAENDPP_InputRange_X64: strcpy(dest, "X64"); break;
    case CAENDPP_InputRange_X128: strcpy(dest, "X128"); break;
    case CAENDPP_InputRange_X256: strcpy(dest, "X256"); break;
    default:
    case CAENDPP_InputRange_UNKN: strcpy(dest, "UNKNOWN"); break;
    }
    return dest;
}
