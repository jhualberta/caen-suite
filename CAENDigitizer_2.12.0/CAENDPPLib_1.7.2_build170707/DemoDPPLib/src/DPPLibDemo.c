        ...
        case DPPDemo_Cmd_SetInputRange:
            {
                ...
                // Show only IRs supported by the board.
                if (currBrd < 0 || currBrd >= MAX_BOARD_NUMBER) {
                    setStatusMessage("Unsupported board number %d", currBrd);
                    break;
                }
                printf("Select new Input Range for channel %d\n\n", currCh);
                for (IR = 0; IR < info[currBrd].InputRangeNum; IR++) {
                    char IRStr[200];
                    printf("  %d) %s\n", IR + 1, getInputRangeString(info[currBrd].InputRanges[IR], IRStr));
                }
                printf("  %d) Abort\n", IR + 1);

                // Ask the user for an IR
                IR = _getch() - '1';
                if (IR < 0 || IR >= info[currBrd].InputRangeNum) {
                    setStatusMessage("User Abort");
                    break;
                }
                InRange = info[currBrd].InputRanges[IR];
...
SetHVConfiguration:
        ...
        case DPPDemo_Cmd_TurnHVOFF:
            ...
            setStatusMessage("HV Channel successfully turned OFF");
        ...
        case DPPDemo_Cmd_IncrActiveChannel:
            if (currCh < connectedChNum - 1) {
                currCh++;
                setStatusMessage("Active channel set to %d", currCh);
            }
            else {
                setStatusMessage("Active channel is already last one.");
            }
            break;
        case DPPDemo_Cmd_DecrActiveChannel:
            if (currCh >= 0) {
                ...
                setStatusMessage("Active channel is already %d.", currCh);
            }
            break;
...
CheckIncreasedHVChannel:
            ...
            currHVBrd = newBrd;
            currHVCh = newCh;
            setStatusMessage("Current HV board: %s_%u. Current HV Channel: %d", info[currHVBrd].ModelName, info[currHVBrd].SerialNumber, currHVCh);
...
CheckDecreasedHVChannel:
        ...
        case DPPDemo_Cmd_IncrActiveBoard:
            if (currBrd < connectedBrdNum - 1) {
                currBrd++;
                setStatusMessage("Active board set to %d", currBrd);
            }
            else {
                setStatusMessage("Active board is already last one.");
            }
            break;
        case DPPDemo_Cmd_DecrActiveBoard:
            if (currBrd >= 0) {
                ...
                setStatusMessage("Active board is already %d.", currBrd);
            }
            break;
        case DPPDemo_Cmd_IncrActiveHisto:
            currHisto++;
            setStatusMessage("Current histogram set to %d", currHisto);
            break;
        case DPPDemo_Cmd_DecrActiveHisto:
            if (currHisto >= 0) {
                ...
                setStatusMessage("Current histogram is already %d.", currHisto);
            }
            break;
