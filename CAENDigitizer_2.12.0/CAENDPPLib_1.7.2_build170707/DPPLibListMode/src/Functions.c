...
int32_t InitDgtzParams(CAENDPP_DgtzParams_t *Params) {
    ...
    strcpy(Params->ListParams.fileName, "UNNAMED"); // Only for savemode file
    Params->ListParams.maxBuffNumEvents = 8192;
    Params->ListParams.saveMask = (DUMP_MASK_TTT | DUMP_MASK_ENERGY | DUMP_MASK_EXTRAS); // Only for savemode file
    ...
    for (ch = 0; ch < MAX_BOARD_CHNUM; ch++) {
        ...
        Params->DPPParams.thr[ch] = 100;        // Trigger Threshold
        Params->DPPParams.nsbl[ch] = 4;         // Number of Samples for Baseline Mean
        Params->DPPParams.nspk[ch] = 0;         // Number of Samples for Peak Mean Calculation
        Params->DPPParams.pkho[ch] = 1000;      // Peak Hold Off
        Params->DPPParams.blho[ch] = 500;       // Base Line Hold Off
        ...
        Params->DPPParams.trgho[ch] = 2300;     // Trigger Hold Off
        Params->DPPParams.twwdt[ch] = 0;        // 
        Params->DPPParams.trgwin[ch] = 0;       //

        // Parameters for X770
        Params->DPPParams.X770_extraparameters[ch].detectorType =
            CAENDPP_DetectorType_Continuous;
        ...
        Params->DPPParams.X770_extraparameters[ch].ResetDetector.ResetDetectionMode =
            CAENDPP_ResetDetectionMode_Internal;
        Params->DPPParams.X770_extraparameters[ch].SaturationHoldoff = 300;
		Params->DPPParams.X770_extraparameters[ch].ResetDetector.thrhold = 100;
        Params->DPPParams.X770_extraparameters[ch].ResetDetector.reslenmin = 2;
        Params->DPPParams.X770_extraparameters[ch].ResetDetector.reslength = 2000;
        ...
        // New DPP-PHA Parameters
        Params->DPPParams.blrclip[ch] = 0;
        Params->DPPParams.dcomp[ch] = 0;
        Params->DPPParams.enskim[ch] = 0;
        Params->DPPParams.eskimlld[ch] = 0;
        Params->DPPParams.eskimuld[ch] = 16383;
        Params->DPPParams.trapbsl[ch] = 0;
    }
    return 0;
}
...
int32_t GetModifiedChannelParameters(int32_t ch, CAENDPP_DgtzParams_t *Params) {
    ...
    while(quit == 0) {
        ...
        printf("7 : Reset Threhold\t\t\t\t= %d\n", Params->DPPParams.X770_extraparameters[ch].ResetDetector.thrhold);
        printf("8 : Reset minimum length\t\t\t= %d\n", Params->DPPParams.X770_extraparameters[ch].ResetDetector.reslenmin);
        printf("9 : Reset hold off\t\t\t\t= %d\n", Params->DPPParams.X770_extraparameters[ch].ResetDetector.reslength);
		printf("---- TRIGGER ----\n");
        ...
        case 'o':
            ...
            break;
        case '1':
			printf("Enter new Analog Datapath:\n\n 1) Continuos Reset 2) Transient Reset\n\n ");
			c = _getch();
            if (c == '1')
                Params->DPPParams.X770_extraparameters[ch].detectorType = CAENDPP_DetectorType_Continuous;
            else if (c == '2')
                Params->DPPParams.X770_extraparameters[ch].detectorType = CAENDPP_DetectorType_Reset;
            break;
		case '2':
			printf("Enter Input Impedance:\n\n 1) 50R 2) 10K\n\n ");
			c = _getch();
            if (c == '1')
                Params->DPPParams.X770_extraparameters[ch].InputImpedance = 0;
            else if (c == '2')
                Params->DPPParams.X770_extraparameters[ch].InputImpedance = 1;
            break;
        case '3':
		...
		case '6':
			...
			c = _getch();
            if (c == '1')
                Params->DPPParams.X770_extraparameters[ch].ResetDetector.ResetDetectionMode = CAENDPP_ResetDetectionMode_Internal;
            else if (c == '2')
                Params->DPPParams.X770_extraparameters[ch].ResetDetector.ResetDetectionMode = CAENDPP_ResetDetectionMode_GPIO;
            break;
		case '7':
            printf("Enter Reset minimum len: (us)");
            if (scanf("%d", &f32) == 1)
                Params->DPPParams.X770_extraparameters[ch].ResetDetector.thrhold = (int32_t)(f32 * 1000.0);
            break;
		case '8':
            printf("Enter Reset threshold: (us)");
            if (scanf("%d", &f32) == 1)
                Params->DPPParams.X770_extraparameters[ch].ResetDetector.reslenmin = (int32_t)(f32 * 1000.0);
            break;
		case '9':
            printf("Enter Reset holdoff: (us)");
            if (scanf("%d", &f32) == 1)
               Params->DPPParams.X770_extraparameters[ch].ResetDetector.reslength = (int32_t)(f32 * 1000.0);
            break;
		case 'U':
...
DPPDemo_Cmd_t getCommad(char c) {
    ...
    case CMD_NONE_CHAR: return DPPDemo_Cmd_None;
    default: break;
    }
    return DPPDemo_Cmd_Unknown;
}
