...
int32_t main() {
    ...
    // Handler to library to control local instance
    int32_t localHandle = -1;

    // Parameters to attach a running remote instance
    char attachIP[16];
    uint16_t attachPort;
    int32_t attachNumBrd;
    int32_t attachBrdIds[MAX_BOARD_NUMBER];
    ...
    // Init the library
    ret = CAENDPP_InitLibrary(&localHandle); // The handle will be used to command the library
    ...
    while (!quit) {
        ...
        // DPPLibrary API used in DPPDemo_Cmd_AttachBoards:
        //   - CAENDPP_AttachBoards(char IP[16], uint16_t port, int32_t *handle, int32_t *numBrd, int32_t *brdIds);
        //   - CAENDPP_GetDPPInfo(int32_t handle, int32_t boardId, CAENDPP_Info_t* info);
        case DPPDemo_Cmd_AttachBoards:
            // Check if a remote istance is already connected.
            if (remoteHandle != -1) {
                setStatusMessage("Already attached to a remote instance.");
                ret = 0;
                break;
            }

            // Ask the user for attach parameters
            /*printf("Enter IP and port to connect (space separated): ");
            if ((scanf("%s %u", attachIP, &attachPort)) != 2) {
                setStatusMessage("Invalid attach parameters.");
                ret = 0;
                break;
            }*/ // HACK !!
            sprintf(attachIP, "10.0.7.172");
            attachPort = 23457;

            // Attach to running instance
            ret = CAENDPP_AttachBoards(attachIP, attachPort, &remoteHandle, &attachNumBrd, attachBrdIds);
            if (ret != CAENDPP_RetCode_Ok) {
                setStatusMessage("Error attach to instance at IP %s:%u: %s", attachIP, attachPort, decodeError(codeStr, ret));
                ret = CAENDPP_RetCode_Ok;
                break;
            }
            ...
            // HACK
            handle = remoteHandle;
            for (i = 0; i < attachNumBrd; i++) {
                bId[i] = attachBrdIds[i];
                // Get Board Info
                ret = CAENDPP_GetDPPInfo(handle, bId[i], &info[i]);
                if (ret != CAENDPP_RetCode_Ok) {
                    setStatusMessage("Error getting board info: %s", decodeError(codeStr, ret));
                    ret = CAENDPP_RetCode_Ok;
                    break;
            ...
            setStatusMessage("Successfully added %d boards", attachNumBrd);

            break; // DPPDemo_Cmd_AttachBoards
        ...
        case DPPDemo_Cmd_Configuration_Set: // Configure Boards
            ...
            break; // DPPDemo_Cmd_Configuration_Set
        
        // DPPLibrary API used in DPPDemo_Cmd_Configuration_Get:
        //   - CAENDPP_GetBoardConfiguration(int32_t handle, int32_t boardId, CAENDPP_AcqMode_t acqMode, CAENDPP_DgtzParams_t dgtz_params);
        case DPPDemo_Cmd_Configuration_Get: // Configure Boards
            if (currBrd < 0) {
                setStatusMessage("Invalid board selected.");
                break;
            }
            ret = CAENDPP_GetBoardConfiguration(handle, bId[currBrd], &acqMode, &dgtzParams);
            if (ret != CAENDPP_RetCode_Ok)
                setStatusMessage("Can't get configuration from board %s_%u: %s", info[currBrd].ModelName, info[currBrd].SerialNumber, decodeError(codeStr, ret));
            else
                setStatusMessage("Get Configuration Succeeded.");
            break;
        ...
        case DPPDemo_Cmd_StopAcq: // Stop Acquisition for all boards
            ...
            if (ret != CAENDPP_RetCode_Ok)
                ...
                AcqRun = FALSE; // TODO should check currCh here...
            }

            if (histo != NULL) {
                FILE *hout = fopen("histo.txt", "w");
                if (hout != NULL) {
                    uint32_t i;
                    for (i = 0; i < (0x1 << 14); i++)
                        fprintf(hout, "%u\n", histo[i]);
        ...
        case DPPDemo_Cmd_SingleHVMonitor:
            ...
            for (brd = 0; brd < connectedBrdNum; brd++) {
                ...
                for (; ch < chmax; ch++) {
                    ...
                    else if ((ret = CAENDPP_GetHVStatusString(handle, bId[brd], u16, msg)) != CAENDPP_RetCode_Ok)
...
CheckDecreasedHVChannel:
        ...
        // Allocate events (if needed)
        // TODO the allocation size should actually be the max set on every board.
        if (Events == NULL) {
            Events = (CAENDPP_ListEvent_t*)calloc(dgtzParams.ListParams.maxBuffNumEvents, sizeof(CAENDPP_ListEvent_t));
            if (Events == NULL) {
                setStatusMessage("Can't allocate events.");
                continue;
            }
        }
        // Allocate histos (if needed)
        if (histo == NULL) {
            histo = (uint32_t*)calloc(0x1 << 14, sizeof(uint32_t));
            if (histo == NULL) {
                setStatusMessage("Can't allocate histogram.");
                continue;
            }
        }
        // TODO, channels loop should be improved here (it assumes they are ordered).
        for (ch = 0; ch < connectedChNum; ch++) {
            uint32_t numEvt, e;
            int32_t chenab;

            ret = CAENDPP_IsChannelEnabled(handle, ch, &chenab);
            if (ret != CAENDPP_RetCode_Ok)
                setStatusMessage("Error getting channel enabled.");
            else if (chenab) {
                ret = CAENDPP_GetListEvents(handle, ch, Events, &numEvt);
                if (ret != CAENDPP_RetCode_Ok)
                    setStatusMessage("Error reading events.");
                else {
                    for (e = 0; e < numEvt; e++) {
                        if (Events[e].Energy >= (0x1 << 14))
                            Events[e].Energy = (0x1 << 14) - 1;
                        if (Events[e].Energy >= 0 && Events[e].Energy < (0x1 << 14) && Events[e].Extras == 0)
                            histo[Events[e].Energy]++;
                        if (Events[e].Extras != 0)
                            continue;
                    }
                }
            }
...
QuitProgram:
    // End the library
    if (localHandle != -1)
        CAENDPP_EndLibrary(localHandle);

    // Free histogram and probes
    if (h1 != NULL) free(h1);
